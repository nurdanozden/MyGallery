 ...
 * atmosferik bir "yer tutucu baskı" üretir. Yalnızca demo modunda kullanılır:
 * `npm run photos` çalıştırıldığı an yerini gerçek karelere bırakır.
 */
...
export function placeholder(
  ...
  const r = seeded(hash(seed))
  const [deep, mid, light] = tint
...
<filter id="grain"><feTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="3" seed="${hash(seed) % 90}"/><feColorMatrix type="saturate" values="0"/></filter>
